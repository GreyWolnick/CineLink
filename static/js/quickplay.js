...
$(document).on('click', '.actor-parent', function() {
    ...
    if ($(this).data('id') === target.id) {
        ...
        $("#win-info").text(`You connected ${start_name} to ${target.name} in ${$("#clock").text()} and only ${clicks} clicks.`);
...
function updateClock() {
    if (isRunning) {
        ...
        var minutes = Math.floor(startTime / 60) % 60;
